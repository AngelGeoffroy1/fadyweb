-- Corriger get_current_booking_status pour utiliser le fuseau horaire Europe/Paris
...
BEGIN
    ...
    IF booking_record.status = 'pending' 
        AND (
            booking_record.booking_date < current_date_paris 
            OR (booking_record.booking_date = current_date_paris AND current_time_paris >= booking_record.booking_time::time)
        ) THEN
        RETURN 'past';
    END IF;
    ...
    IF booking_record.status = 'confirmed'
        AND (
            booking_record.booking_date < current_date_paris 
            OR (booking_record.booking_date = current_date_paris AND current_time_paris >= (booking_record.booking_time::time + (booking_record.total_duration || ' minutes')::interval))
        ) THEN
        RETURN 'completed';
    END IF;
    ...
    IF booking_record.status = 'confirmed'
        AND booking_record.booking_date = current_date_paris 
        AND current_time_paris >= booking_record.booking_time::time
        AND current_time_paris < (booking_record.booking_time::time + (booking_record.total_duration || ' minutes')::interval) THEN
        RETURN 'en_cours';
    END IF;
